...
def get_time():
    up_time = get('10.10.1.1', ['1.3.6.1.2.1.1.3.0'])
...
def get(target, oids, credentials = hlapi.CommunityData('ciscolab'), port=161, engine=hlapi.SnmpEngine(), context=hlapi.ContextData()):
...
def get_bulk_auto_int_up_or_down(target = '10.10.1.1', oids = '[1.3.6.1.2.1.2.2.1.8]', credentials = 'ciscolab', count_oid = '1.3.6.1.2.1.2.1.0', start_from=0, port = 161, engine=hlapi.SnmpEngine(), context=hlapi.ContextData()):
    count = get(target, [count_oid], credentials, port, engine, context)[count_oid]
    return get_bulk(target, oids, credentials, count, start_from, port, engine, context)
